...
def scrape_private():
    ...
    with get_db_connection() as conn:
        conn.execute('DELETE FROM deals WHERE date_found=? AND deal_source="private"', (today,))
        conn.commit()
    ...
    while True:
        ...
        try:
            ...
            for item in items:
                ...
                print(f"[{p}] Procesando Privada: {name}...")
                ...
                if is_acc:
                    with get_db_connection() as conn:
                        conn.execute('INSERT OR REPLACE INTO deals (philibert_name, price, old_price, url, date_found, is_accessory, is_expansion, deal_source, image_local) VALUES (?,?,?,?,?,?,?,?,?)', (name, price, old_p, u, today, 1, 0, 'private', img_local))
                    continue
                ...
                if cached:
                    ...
                    id_b, conf, real_n = fetch_bgg_id(name, u, source='private')
                ...
                with get_db_connection() as conn:
                    conn.execute('INSERT OR REPLACE INTO deals (philibert_name, price, old_price, url, date_found, is_accessory, is_expansion, deal_source, image_local) VALUES (?,?,?,?,?,?,?,?,?)', (name, price, old_p, u, today, 0, any(k in name.lower() for k in ['extens','expans','pack']), 'private', img_local))
