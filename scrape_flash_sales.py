...
# Palabras clave para IGNORAR (Filtro rápido)
IGNORE_KEYWORDS = [
...
GENERIC_TITLES = {
    'le jeu de plateau', 'the board game', 'jeu de plateau', 'board game', 
...
TRANSLATIONS = {
    ...
    'chanson': 'songs', 'franco': 'french', 'francaise': 'french', 'chroniques': 'chronicles',
    'plateau': 'board'
}
...
def is_rare_word(w, orig_word=""):
    ...
    common_words = {'the', 'of', 'in', 'and', 'to', 'for', 'with', 'on', 'at', 'by', 'from', 'an', 'is', 'it', 'was', 'were', 'jeu', 'board', 'game'}
...
def log_failed_match_html(name, url, assigned_id, candidates_dict):
    ...
    if not os.path.exists(LOG_HTML_PATH):
        html = "<html><head><meta charset='utf-8'><style>table{border-collapse:collapse;width:100%}th,td{border:1px solid #ccc;padding:8px}tr.best-score{background-color:#d4edda}tr.best-conf{border:3px solid #ffc107;font-weight:bold}tr.assigned{border:4px solid #007bff !important}span.badge{background:#007bff;color:#fff;padding:2px 4px;border-radius:3px;font-size:0.8em}a{text-decoration:none;color:#007bff}a:hover{text-decoration:underline}</style></head><body>"
    ts = datetime.datetime.now().strftime('%Y-%m-%d %H:%M')
    html += f"<h3><a href='{url}' target='_blank'>[FLASH] {name}</a> ({ts})</h3>"
    ...
    for c in sorted(candidates, key=lambda x: x['score'], reverse=True)[:10]:
        cls = []; is_a = str(c['id']) == str(assigned_id)
        if c['score'] == max_score: cls.append("best-score")
        if c['conf'] == max_conf: cls.append("best-conf")
        if is_a: cls.append("assigned")
        badge = " <span class='badge'>ASIGNADO</span>" if is_a else ""
        bgg_url = f"https://boardgamegeek.com/boardgame/{c['id']}"
        html += f"<tr class='{' '.join(cls)}'><td><a href='{bgg_url}' target='_blank'>{c['id']}</a>{badge}</td><td>{c['name']}</td><td>{c['score']:.1f}</td><td>{c['conf']:.1f}%</td></tr>"
    html += "</table><hr>"
...
def log_failed_match(source, name, url, best_id, best_conf, candidates_dict):
    ...
    with open(LOG_PATH, 'a', encoding='utf-8') as f:
        f.write(f"\n--- {datetime.datetime.now()} | [{source.upper()}] '{name}' ---\nURL: {url}\nASIGNADO ID: {best_id} | Conf: {best_conf:.1f}%\n" + "-"*60 + "\n")
...
def norm_plain(s): return remove_accents(norm_acc(s))

def fetch_bgg_id(game_name, phili_url, source='flash'):
    ...
    # ESTRATEGIAS REFINADAS: Comillas para precisión exacta (Sa-Rê)
    raw_strategies = [f'"{game_name.strip()}"', game_name.strip(), phili_plain] + phili_parts
    
    # Si el nombre es muy corto, permitimos palabras de 2 letras
    ...
    for q in strategies:
        time.sleep(1)
        try:
            res = requests.get(url, params={"query": q, "type": "boardgame,boardgameexpansion,boardgameaccessory"}, headers=HEADERS, timeout=10)
            ...
            for item in soup.find_all('item')[:20]:
                ...
                if b_clean == target_clean: 
                    score += 2000
                    if len(target_clean) < 10: score += 5000 # SUPER BONUS para nombres cortos exactos (Sa-Re)
                
                # VETO POR LONGITUD DISPAR (Evita que Sa-Re coincida con nombres larguísimos)
                len_diff = abs(len(b_clean) - len(target_clean))
                if len(target_clean) < 10 and len_diff > 12: score -= 6000 # Penalización más radical
                for bw in b_clean_words:
                    if bw not in target_clean_words and all(tw not in bw for tw in target_clean_words): score -= 300
                if is_base_explicit:
                    if item.get('type') == 'boardgameexpansion': score -= 4000
                    elif item.get('type') == 'boardgame': score += 1500
                elif is_ext:
                    if item.get('type') == 'boardgame': score -= 1500
                    elif item.get('type') == 'boardgameexpansion': score += 800
    ...
    if best_confidence < 90:
        if is_philibert_excluded(phili_url):
            return None, 0, "" 
...
def fetch_details(bgg_id):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # Nombre Original y Dependencia Idioma
...
def scrape_flash():
    ...
    with get_db_connection() as conn: conn.execute('DELETE FROM deals WHERE date_found=? AND deal_source="flash"', (today,)); conn.commit()
    url_base = "https://www.philibertnet.com/fr/flash-sales"
    p = 1; seen = set()
    while True:
        ...
        try:
            ...
            for item in items:
                ...
                # FILTRO RÁPIDO JDR / FIGS / ACCESORIOS
                is_ignored = any(k.lower() in name.lower() for k in IGNORE_KEYWORDS) or any(k.lower() in u.lower() for k in ['jeu-de-role', 'figurine', 'wargame', 'warhammer'])
                is_acc = any(k.lower() in name.lower() for k in ['Rangement','Organizer','Sleeves','Tapis','Mat','Neoprene','Token','Tokens','Card Holder','Standees','Deck Box','Extra Player Pack']) or ("puzzle" in name.lower() and any(x in name.lower() for x in ['pièces','pieces']))
                ...
                if is_acc:
                    with get_db_connection() as conn: conn.execute('INSERT OR REPLACE INTO deals (philibert_name, price, old_price, url, date_found, is_accessory, is_expansion, deal_source) VALUES (?,?,?,?,?,?,?,?)', (name, price, old_p, u, today, 1, 0, 'flash'))
                    continue
                ...
                if cached: id_b, conf, real_n = cached[0], cached[1], name
                    ...
                    if id_b is None: continue # Ignorado tras deep check
                ...
                with get_db_connection() as conn:
                    conn.execute('INSERT OR REPLACE INTO deals (philibert_name, price, old_price, url, date_found, is_accessory, is_expansion, deal_source) VALUES (?,?,?,?,?,?,?,?)', (name, price, old_p, u, today, 0, any(k in name.lower() for k in ['extens','expans','pack']), 'flash'))
...
if __name__ == "__main__": scrape_flash()
