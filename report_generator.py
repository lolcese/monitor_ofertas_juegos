...
def generate_report():
    ...
    with sqlite3.connect(BGG_CACHE_DB) as conn:
        ...
        WHERE d.date_found >= (SELECT date(MAX(date_found), '-1 day') FROM deals)
        ...
        for k, l in mm_lbls.items():
            ...
            if cnt > 0:
                ...
                # CORRECCIÓN COLOR BACKDOOR: Usamos el nombre de clase correcto badge-mm-backdoor
        ...
        for row in rows:
            ...
            # Limpiar "(Clearance)" y " - Occasion" para que el reporte se vea más limpio
            p_name = re.sub(r'\(Clearance\)| - Occasion', '', p_name, flags=re.I).strip()
            ...
            # Limpiar el "Cargando..." molesto si no hay ranking (usar "-" en su lugar)
        ...
        function filterTable(){
            var i=document.getElementById("searchInput"),f=i.value.toUpperCase(),tr=document.getElementById("offersTable").getElementsByTagName("tr");
            for(var j=1;j<tr.length;j++){
                var v=false,td=tr[j].getElementsByTagName("td");
                for(var k=0;k<td.length;k++){
                    if(td[k]&&(td[k].textContent||td[k].innerText).toUpperCase().indexOf(f)>-1){v=true;break}
                }
                tr[j].style.display=v?"":"none"
            }
        }
        ...
        function sortTable(n){
            var t=document.getElementById("offersTable"),b=t.tBodies[0],r=Array.from(b.rows),d=t.getAttribute("sort-dir-"+n)=="asc"?-1:1;
            r.sort((a,b)=>{
                var x=a.cells[n].getAttribute("data-sort")||a.cells[n].textContent.trim(),y=b.cells[n].getAttribute("data-sort")||b.cells[n].textContent.trim();
                if(!isNaN(parseFloat(x))&&!isNaN(parseFloat(y)))return(parseFloat(x)-parseFloat(y))*d;
